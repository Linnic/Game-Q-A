...
//Top 10 names & grades
var rank = [{"name":"Midy","grade":999},
					{"name":"Sida","grade":750},
					{"name":"Uxior","grade":100},
					{"name":"Frich","grade":40},
					{"name":"Ruki","grade":2},
					{"name":"null","grade":0},
					{"name":"null","grade":0},
...
//change rank					
function ranking(name,grade){
	var count=0;
	for(var t=0;t<10;t++){
		if(grade>rank[t].grade){
			count+=1;
			if(count==1 && t!=9){
				for(var j=9;j>t;j--){
//					console.log(j);
...
var index = getRandomArray(0, 34, 35);
console.log(index);
...
//		socket.emit('sysmessage', 'SERVER', 'You have connected'); 
		onlinepeople.splice(onlinepeople.length, 1, username);
//		socket.broadcast.emit('sysmessage', 'SERVER', username + ' is online');
		socket.send('Question: ',QandA[index[now]]['q']);
		socket.emit('top10grade', rank);
		io.sockets.emit('whoisonline', onlinepeople);
...
//		io.sockets.emit('sysmessage', 'SERVER', socket.username + ' has left the building');
		onlinepeople.splice(onlinepeople.indexOf(socket.username), 1);
		io.sockets.emit('whoisonline', onlinepeople);
	});
	
	socket.on('sendgrade', function(data) {
		console.log(data);
		ranking(socket.username,data);
//		console.log(rank);
		io.sockets.emit('top10grade', rank);
	});
